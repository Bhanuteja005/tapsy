...
const Task = () => {
  ...
  useEffect(() => {
    if (mode === "update") {
      ...
      fetchData(config, { showSuccessToast: false }).then((data) => {
        setTask(data.task);
        // Ensure title is included when setting form data
  ...
  const handleSubmit = e => {
    ...
    // Adjust the config to include the title in the data sent to the backend
    const dataToSend = { ...formData };
    if (mode === "add") {
      const config = { url: "/tasks", method: "post", data: dataToSend, headers: { Authorization: authState.token } };
      fetchData(config).then(() => {
        navigate("/");
      });
    }
    else {
      const config = { url: `/tasks/${taskId}`, method: "put", data: dataToSend, headers: { Authorization: authState.token } };
      fetchData(config).then(() => {
        navigate("/");
      });
    }
  ...
  <>
    ...
    <div className="mb-4">
      ...
      <input type="text" name="title" id="title" value={formData.title || ''} placeholder="Task title.." onChange={handleChange} className="input-class-name"  style={{ marginLeft:'10px', padding: '10px', border: '1px solid #ccc', borderRadius: '4px' }}/>
      {fieldError("title")}
    </div>
    {/* Existing code for description */}
    <div className="mb-4">
