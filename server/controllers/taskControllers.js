...
exports.postTask = async (req, res) => {
  try {
    ...
    if (!title || !description) {
      return res.status(400).json({ status: false, msg: "Both title and description of the task are required" });
    }
...
exports.putTask = async (req, res) => {
  try {
    const { title, description } = req.body;
    if (!title && !description) {
      return res.status(400).json({ status: false, msg: "At least one of title or description must be provided for update" });
    }
